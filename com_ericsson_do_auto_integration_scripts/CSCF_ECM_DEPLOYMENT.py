...
log = Logger.get_logger('CSCF_ECM_DEPLOYMENT.py')
...
# method to get folder name that is named upon cscf software folder
def get_vnfd_id_cscf_nodes():
    get_vnfd_id_ims_nodes('CSCF', 'deployCSCF')


# new changes for IMS nodes
def transfer_cscf_software():
    transfer_node_software_vnflcm('CSCF', 'deployCSCF')


# Not in use after auto download from Node ARM Repo
"""
def unpack_cscf_software():
    sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
    cscf_version = sit_data._SIT__cscf_version
    hot_file = 'vcscf_hot.yaml'
    unpack_node_software('CSCF', cscf_version, 'CSCF_Software_complete.tar', 'CSCF_Software_resources.tar', hot_file)

def cscf_workflow_deployment():
    try:
        log.info('start CSCF workflow bundle install on LCM')
        Report_file.add_line(Report_file, 'CSCF Workflow  bundle install on LCM begins...')
        sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
        ...
        cscf_software_path = r'/var/tmp/' + sit_data._SIT__cscf_version

        connection = Server_connection.get_connection(Server_connection, server_ip, username, password)

        command = 'cd ' + cscf_software_path +' ;ls -ltr | grep -i .rpm'
        ...
        command = 'sudo -i wfmgr bundle install --package={}/{}'.format(cscf_software_path, rpm_name)
        ...
        connection.close()
        log.info('Error CSCF workflow bundle install on LCM '+str(e))
        Report_file.add_line(Report_file, 'Error CSCF workflow bundle install on LCM '+str(e))
        assert False

def cscf_so_files_transfer():
    sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
    cscf_software_path = sit_data._SIT__cscf_software_path
    so_files_transfer(cscf_software_path)
"""
...
def generate_ssh_key():
    try:
        ...
        ServerConnection.get_file_sftp(connection,
                                       r'/vnflcm-ext/current/vnf_package_repo/' + vnfd_id + '/Resources/VnfdWrapperFiles/VNFD_Wrapper.json',
                                       r'com_ericsson_do_auto_integration_files/VNFD_Wrapper_CSCF.json')
        file = 'com_ericsson_do_auto_integration_files/VNFD_Wrapper_CSCF.json'
        workflow_config_dir = Json_file_handler.get_any_json_attr(Json_file_handler, file, ['instantiateVnfOpConfig'],
                                                                  'workflow_config_dir')
        ...
        connection.close()
        log.error('Error CSCF ssh-key generation ' + str(e))
        Report_file.add_line('Error CSCF ssh-key generation ' + str(e))
        assert False


"""
def create_cscf_flavours():

    log.info('start creating cscf flavors')
    Report_file.add_line(Report_file, 'start creating cscf flavors')
    ...
    cscf_flavors = sit_data._SIT__cscf_flavors
    flavors_list = cscf_flavors.split(',')
    for flavor_name in flavors_list:
        ...
        if flavor_exists:
            ...
            name = flavor_name[3:]
            if '32-250-8' in name:                
                update_any_flavor_file(name,4,32768,250,tenant_name)                
            elif '32-0-8' in name:
                update_any_flavor_file(name,4,32768,0,tenant_name)                
            else:
                log.warn('Flavor name does not match with requirements , please check the confluence for CSCF deployment '+flavor_name)
                Report_file.add_line(Report_file, 'Flavor name does not match with requirements , please check the confluence for CSCF deployment '+flavor_name)
                assert False

            update_transfer_flavour_file()
            create_flavour('flavour.json','flavour_transfer.json',name)

    log.info('Finished creating cscf flavors')
    Report_file.add_line(Report_file, 'Finished creating cscf flavors')

def register_cscf_images():
    try:

        log.info('start register cscf images')
        Report_file.add_line(Report_file, 'start register cscf images')
        sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
        ecm_host_data = Initialization_script.get_model_objects(Initialization_script, 'ECM_CORE')
        ...
        cscf_image_names = sit_data._SIT__cscf_image_names
        cscf_image_ids = sit_data._SIT__cscf_image_ids
        image_names = cscf_image_names.split(',')
        image_ids = cscf_image_ids.split(',')
        ...
        log.info('Finished register cscf images')
        Report_file.add_line(Report_file, 'Finished register cscf images')
        connection.close()
    except Exception as e:
        connection.close()
        log.error('Error register cscf images '+str(e))
        Report_file.add_line(Report_file, 'Error register cscf images '+str(e))
        assert False"""


def update_cscf_node_onboard_file():
    ...
    file_data = Json_file_handler.get_json_data(Json_file_handler,
                                                r'com_ericsson_do_auto_integration_files/VNFD_Wrapper_CSCF.json')
    global package_name
    package_name = Common_utilities.get_name_with_timestamp(Common_utilities, 'TEST_CSCF_PACKAGE_UPLOAD')
    sit_data._SIT__name = package_name
    log.info('CSCF ONBOARD PACKAGE NAME ' + package_name)
    Report_file.add_line('CSCF ONBOARD PACKAGE NAME ' + package_name)
    onboard_file_name = 'onboard_cscf.json'
    upload_file = vnfd_id + '.zip'
    update_cscf_onboard_file(r'com_ericsson_do_auto_integration_files/' + onboard_file_name, file_data, package_name)
...
def onboard_cscf_package():
    sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
    vnfd_id = sit_data._SIT__ims_vnfd_id
    upload_file = vnfd_id + '.zip'
    onboard_node_package('onboard_cscf.json', upload_file, package_name)


def verify_cscf_package():
    log.info('start verifying the onboarded cscf package')
    sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
    ecm_host_data = Initialization_script.get_model_objects(Initialization_script, 'ECM_CORE')
    ecm_server_ip = ecm_host_data._Ecm_core__ECM_Host_Blade_IP
    ecm_username = ecm_host_data._Ecm_core__ECM_Host_Blade_username
...
def update_cscf_node_deploy_file():
    sit_data = SIT_initialization.get_model_objects(SIT_initialization, 'SIT')
    ecm_host_data = Initialization_script.get_model_objects(Initialization_script, 'ECM_CORE')
    vnfd_id = sit_data._SIT__ims_vnfd_id
    node_name = vnfd_id + '_' + str(random.randint(0, 999))
    node_name = node_name.replace('.', '_')
    file_name = 'deploy_cscf.json'
    ...
    file_data = Json_file_handler.get_json_data(Json_file_handler,
                                                r'com_ericsson_do_auto_integration_files/VNFD_Wrapper_CSCF.json')
    update_cscf_deploy_file(r'com_ericsson_do_auto_integration_files/' + file_name, file_data, node_name)
...
def deploy_cscf_package():
    deploy_node('deploy_cscf.json')


def verify_cscf_deployment():
    try:
        ...
        # file_data = Json_file_handler.get_json_data(Json_file_handler,r'com_ericsson_do_auto_integration_files/VNFD_Wrapper_CSCF.json')
